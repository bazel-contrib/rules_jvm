...
As noted above, if you are building Bazel rules which require Java parts, and hence Java dependencice, it 
can be useful to freeze these dependencies. This process captures the list of dependencies into a zip file. 
This zip file is distributed with your Bazel rulee. This makes your rule more hermetic, your rule no longer 
...
```
2. Run `./bin/frozen-deps.py --repo <repo name> --zip <path/to/dependency.zip>`. The `<repo name>` matches the name used
for the `maven_install()` rule above. This will pin the dependencies then collect them into the zip file. 
...
NOTE: If you need to generate the compat_repositories for the dependencies, usually because your rule depends on another
rule which is still using the older compat repositories, you need to make the following changes and abide by the
following restrictions.

1. Add the `generate_compat_repositories = True,` attribute to the original `maven_install()` rule.
2. In step 2, add the parameter `--zip-repo workspace_deps` to match the name used in the `zip_repository()` rule (step 4).  
If you don't supply this, it uses the base name of the zip file, which may not be what you want. 
