...
/**
 * Consumer that uses multiple inline functions from the provider.
 * This should get transitive dependencies from all the inline functions it uses.
 */
class Consumer {

    fun useArrayListInline(): String {
        val result = withArrayList<String> { list ->
            list.add("test")
            list.add("data")
        }
