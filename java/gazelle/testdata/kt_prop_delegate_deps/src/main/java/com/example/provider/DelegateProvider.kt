...
/**
 * Property delegates that use external dependencies. Any code that uses these properties should
...
class DelegateProvider {
  ...
  /**
   * Lazy property delegate that uses external dependencies. The lazy block uses Gson, so consumers
   * should get Gson transitively.
   */
  val jsonProcessor: Gson by lazy { Gson() }

  /**
   * Observable property delegate that uses external dependencies. The observer uses ArrayList, so
   * consumers should get java.util transitively.
   */
  var dataList: List<String> by
      Delegates.observable(emptyList<String>()) { _, oldValue, newValue ->
